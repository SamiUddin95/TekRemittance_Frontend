...
export class ApprovedQueueListComponent implements OnInit {
  ...
  private loadApprovedData(agentId: string): void {
    console.log('Loading approved data for agent:', agentId);
    this.isLoading = true;
    ...
    this.disbursementService.getDataByApproved(agentId, pageNumber, pageSize).subscribe({
      next: (response) => {
  ...
  repair(_row: ApprovedRow): void { /* placeholder */ }
  revert(_row: ApprovedRow): void { /* placeholder */ }
}
